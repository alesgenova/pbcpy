...
class Cell(object):
    ...
    def __eq__(self, other) :
        """
        method to expose == operator to Cell object. It is general and works for cells with different units.
        """
...
class pbcarray(np.ndarray):
    ...
    def __getitem__(self, sli_in):
        ...
        # the slice in input can as well be an integer, or a tuple of integers/slices with length smaller than the rank of the array.
        print(sli_in)
        ...
        for isl, sl in enumerate(sli):
            ...
            if isinstance(sl,int):
                ...
                if True :
                    slice_tup = noneslicelist.copy()
                    if step < 0:
                        slice_tup[isl] = slice(start+roll,stop+roll,step_)
                    else :
